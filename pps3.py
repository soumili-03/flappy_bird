...
while True:
    ...
    for event in pygame.event.get():
        ...
        if event.type==pygame.KEYDOWN:
            if event.key==pygame.K_SPACE and game_active and game_active2:
                    bird_move=0
                    bird_move-=9
                    flap_sound.play()
            if event.key==pygame.K_SPACE and game_active == False and game_active2==False:        
                    bird_rect.center=(100,345)#to detect any collision around the bird
                    bird_move=0
                    pipe_list.clear()
                    game_activate=True
        if event.type ==SPAWNPIPE and game_active and game_active2:
    ...
    if game_active2:
        ...
        game_active2=check_collision(pipe_list )
    else:
